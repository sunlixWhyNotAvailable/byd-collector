...
INSERT OR IGNORE INTO storage_meta (schema_family, format_version, created_at_ms)
VALUES ('main_telemetry', 2, CAST(strftime('%s', 'now') AS INTEGER) * 1000);
...
CREATE TABLE IF NOT EXISTS decoded_value_dictionary (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    value TEXT COLLATE BINARY NOT NULL UNIQUE
);
...
CREATE TABLE IF NOT EXISTS normalized_history_field_catalog (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    normalizer_id TEXT NOT NULL,
    catalog_version TEXT NOT NULL,
    UNIQUE (field_key, category, value_type, unit, source_keys, normalizer_id, catalog_version)
);
...
CREATE TABLE IF NOT EXISTS vehicle_state_history (
    ...
    value_number REAL,
    value_bool INTEGER CHECK (value_bool IN (0, 1) OR value_bool IS NULL),
    quality_code INTEGER NOT NULL CHECK (quality_code BETWEEN 0 AND 4),
    source_poll_id INTEGER,
    observed_at_ms INTEGER NOT NULL,
    changed_at_ms INTEGER NOT NULL,
    FOREIGN KEY (field_id) REFERENCES normalized_history_field_catalog(id),
    FOREIGN KEY (source_poll_id) REFERENCES polls(id)
);
...
CREATE INDEX IF NOT EXISTS idx_vehicle_state_history_field_id
ON vehicle_state_history(field_id, id);

CREATE INDEX IF NOT EXISTS idx_polls_session_id
ON polls(session_id, id);
