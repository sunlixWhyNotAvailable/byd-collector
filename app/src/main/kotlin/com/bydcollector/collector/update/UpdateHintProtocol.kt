...
internal object UpdateHintProtocol {
    ...
    const val MAX_SIZE_PERCENT = 1_000
    const val MAX_DIMENSION_PX = 100_000
...
internal data class UpdateHintRecord(
    ...
    fun isValid(): Boolean {
        if (protocolVersion != UpdateHintProtocol.VERSION ||
            ...
            processSessionId.isBlank() ||
            processSessionId.length > UpdateHintProtocol.MAX_ID_LENGTH ||
            eventId.length > UpdateHintProtocol.MAX_ID_LENGTH ||
            revision < 0L) return false
        if (phase == UpdateHintPhase.NONE) return true
        if (eventId.isBlank() || requestedAtElapsedNanos <= 0L || displayId != 0 ||
            preferredSizePercent !in 1..UpdateHintProtocol.MAX_SIZE_PERCENT ||
            preferredWidthPx !in 1..UpdateHintProtocol.MAX_DIMENSION_PX ||
            preferredHeightPx !in 1..UpdateHintProtocol.MAX_DIMENSION_PX) return false
        return when (phase) {
            UpdateHintPhase.NONE -> true
            UpdateHintPhase.PENDING -> expiresAtElapsedMs == 0L
            UpdateHintPhase.VISIBLE -> expiresAtElapsedMs > 0L
        }
    }
    ...
    fun isActive(nowElapsedMs: Long, nowElapsedNanos: Long): Boolean = when (phase) {
        ...
        UpdateHintPhase.PENDING -> nowElapsedNanos >= requestedAtElapsedNanos &&
            nowElapsedNanos - requestedAtElapsedNanos <=
