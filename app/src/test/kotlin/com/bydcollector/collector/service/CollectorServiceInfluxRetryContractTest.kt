...
class CollectorServiceInfluxRetryContractTest {
    ...
    fun independentInfluxRetryUsesTheExistingServiceLifecycle() {
        ...
        val executeInflux = service.substringAfter("private fun executeInflux")
            .substringBefore("private fun settleInfluxWork")

        assertTrue(normalizedWrite.contains("requestInfluxCycle(newData = true)"))
        assertTrue(service.contains("private val influxRetryTask"))
        assertTrue(influxPath.contains("influxCycleDemand.tryAcquire"))
        assertTrue(influxPath.contains("synchronized(influxQueueLock)"))
        assertTrue(influxPath.contains("requestRevision == influxRequestRevision"))
        assertTrue(influxPath.contains("followUpDemand = influxCycleDemand.pending"))
        assertTrue(influxPath.contains("val coordinator = influxCoordinator"))
        assertTrue(influxPath.contains("influxCoordinator === coordinator"))
        assertTrue(influxPath.contains("coordinator.runOneCycle(force = false, isCurrent = isCurrentRequest)"))
        assertTrue(influxPath.contains("expectedGeneration = request.generation"))
        assertTrue(service.contains("coordinator.startExport(isCurrent = isCurrentRequest)"))
        assertTrue(service.contains("coordinator.resumeExport(isCurrent = isCurrentRequest)"))
        ...
        assertTrue(service.contains("settleInfluxWork()"))
        assertTrue(service.contains("afterSettled?.invoke()"))
        assertTrue(executeInflux.indexOf("settleInfluxWork()") < executeInflux.indexOf("afterSettled?.invoke()"))
        assertTrue(executeInflux.contains("isInfluxSubmissionCurrent(expectedGeneration, influxWorkGeneration.get())"))
        val failedAction = executeInflux.substringAfter("onFailedAction =")
        assertTrue(failedAction.contains("isInfluxSubmissionCurrent(expectedGeneration, influxWorkGeneration.get())"))
        ...
        assertTrue(influxPath.contains("influxDiagnosticStateDetails(request.generation, queued = true)"))
        assertTrue(influxPath.contains("influxCycleDemand.settle(request, represented)"))
        assertTrue(generation.contains("influxWorkGeneration.incrementAndGet()"))
        assertTrue(generation.contains("influxCycleDemand.invalidate()"))
        assertTrue(generation.contains("influxRequestRevision += 1"))
