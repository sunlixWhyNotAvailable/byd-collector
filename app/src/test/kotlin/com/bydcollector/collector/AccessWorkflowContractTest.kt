...
class AccessWorkflowContractTest {
    ...
    fun coldAndManualUiFlowsUseTheRequiredModesAndOrdering() {
        ...
        val manualAdb = method(source, "private fun requestAdbAuthorizationFlow", "override fun onRequestPermissionsResult")
        assertTrue(manualAdb.contains("AccessCheckMode.FORCE"))
        ...
        assertTrue(source.contains("private fun startupHardFlowBlocked(): Boolean"))
        assertTrue(source.contains("runtimePermissionRequestInFlight = runtimePermissionRequestInFlight"))
        ...
        assertInOrder(logcatStart, "val task = Runnable", "DiagnosticLogRecorder.start")
        assertInOrder(logcatStart, "DiagnosticLogRecorder.start", "diagnosticsExecutor.execute(task)")
    ...
    fun serviceUsesExactActiveWorkAndIndependentFiveMinuteCadence() {
        ...
        assertTrue(autoStart.contains("clearsManualStops(action) && (demand.main || demand.debug) && settings.hasActiveAccessWork()"))
