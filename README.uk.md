...
Збірка v3.0.0 використовує гібридний callback-збір. Після отримання придатних callback-подій підтримувані поля переходять на оновлення від автомобіля з контрольним читанням кожні п'ять секунд; непідтверджені поля залишаються на опитуванні. Power, передача та SOC зберігають звичайне швидке читання. Кожна отримана callback-подія, навіть із незмінним значенням, окремо зберігається в наявних базах SQLite. Підтримка параметрів і фонове відновлення можуть відрізнятися залежно від прошивки авто.

Буфер callback-подій обмежений: кожен потік має спільний ліміт 16 MiB оперативної пам'яті для даних у черзі та під час передачі; з них до 12 MiB може утримувати транспорт. Необроблені пакети через п'ять секунд або при досягненні ліміту переходять на диск; файли опитування й callback-подій ділять наявний ліміт резервного spool 128 MiB. Коли авто й екран увімкнені, імпорт у SQLite дозується, якщо дискова черга не зростає; коли авто вимкнене, а застосунок продовжує працювати, імпорт наздоганяє чергу без такого обмеження. Втрати через ліміт або помилки запису явно фіксуються; для даних, що ще в пам'яті, збереження після kernel reboot не гарантується.

Помилки розрахунку енергії повторно обробляються з довготривалої черги SQLite без зупинки запису сирих даних. Архівація основної бази відкладається до спорожнення цієї черги. При впорядкуванні нормалізованих даних показники поточного завантаження системи вважаються новішими за збережені показники попереднього, навіть якщо системний годинник перевели назад.
...
| Всі дані | Дослідницький збір 23 069 активних сигнатур читання, сирі значення та запис змін |
...
- Відображає SOC, залишок енергії батареї, SOH, пробіг, температури салону й батареї, заряджання/розряджання, запас ходу, різницю напруги комірок і підсумки категорій. Ці KPI-картки отримують дані через окремий читач, коли авто увімкнене, навіть якщо основний і вторинний збір зупинені.
...
- Читає 23 069 активних сигнатур зі збереженого дослідницького каталогу 23 083 визначень. Сім високочастотних або нецільових native-полів виключені і з опитування, і з підписок; їхні попередні дані залишаються доступними.
...
Таблиця окремо показує витрачену з АКБ енергію, рекуперовану під час руху та баланс АКБ (витрачено мінус рекуперовано). Середня витрата рахується за балансом і може бути від’ємною. Ці розрахункові значення також доступні в категорії «Батарея» для MQTT та InfluxDB. Пропуски не заповнюються припущеннями про потужність. Дані про якість показників залишаються у збереженій телеметрії. Старі поїздки зберігають попереднє значення енергії як баланс АКБ та відповідну середню витрату; недоступні витрачена й рекуперована енергія показуються прочерками.

Старі поїздки перераховуються у фоні, якщо поточна основна база ще містить повну придатну телеметрію для них. Якщо історія неповна, попередній баланс зберігається, а середня витрата рахується з цього балансу та дистанції поїздки. Архівні бази для цього перерахунку не використовуються.

`Поточна поїздка` поруч зі стисненням відкриває метрики й мапу активної сесії. Вікно оновлюється, поки видиме, без скидання положення чи масштабу мапи. Наявна іконка фінішного прапорця позначає останню довірену координату як `Поточна позиція`, або `Остання відома позиція`, якщо дані застаріли чи після них є прогалина GPS. Без довіреної координати прапорець не показується. Використовується чинне оновлення маршруту, без додаткового збору GPS чи записів у базу. Після завершення у вікні залишається ця сама поїздка з її фактичним фінішем.
...
Автоматична перевірка оновлень починається через 30 секунд і працює у фоні без відкриття Collector. Невдалі перевірки повторюються через 30, 60, 120, а далі кожні 300 секунд від завершення невдалої спроби; успішна перевірка зупиняє повтори. Вимкнення екрана призупиняє лише перевірку оновлень, а не збір телеметрії. Наступне пробудження екрана починає новий 30-секундний цикл. Відкриття або згортання Collector не скидає цей таймер.

Коли Collector видимий, доступне оновлення пропонується у самому застосунку. Закриття пропозиції оновлення призупиняє автоматичні перевірки на годину, якщо нове пробудження не почне свіжий цикл раніше. Ручна перевірка залишається доступною й перевіряє останній опублікований реліз; якщо актуальна перевірка вже виконується, вона використовує той самий запит замість запуску ще одного.

`Віджет-підказка нової версії` початково ввімкнений. Якщо фонова перевірка знаходить оновлення, підказка може з'явитися поверх інших застосунків на 10 секунд. Натисніть її, щоб відкрити `Опції` та збережену пропозицію оновлення, або закрийте хрестиком. Повернення до Collector прибирає його підказку без повторної перевірки; згортання не повторює вже показану підказку чи пропозицію. Кнопка налаштувань поруч із перемикачем дозволяє змінити розмір, прозорість, заокруглення й колір рамки та риски; початковий колір акценту — зелений.

Для підказки потрібен дозвіл Android на показ поверх інших застосунків, який запитується через системний екран налаштувань. Відмова не викликає повторних запитів; для нової спроби знову ввімкніть підказку. Якщо показ не вдався, збережена пропозиція залишається доступною без нової мережевої перевірки. Без дозволу звичайні пропозиції оновлення та збір працюють і далі. Із сумісними версіями BYD HUD та BYD Extend підказки займають доступні місця на екрані й за потреби тимчасово зменшуються; кожна зберігає власний час показу та користувацькі налаштування оформлення.
...
Перед поширенням у діагностичних копіях маскуються розпізнані облікові дані, ідентифікатори Wi-Fi мереж (SSID/BSSID), явно позначені координати та приватні ідентифікатори автомобіля/чату/акаунта. Версії застосунку й прошивки, назви автомобілів, хости/IP-адреси та порти залишаються читабельними для діагностики. Оригінальні логи й архіви баз не змінюються. **Ця фільтрація не гарантує анонімності повносистемних логів.**
