...
| Напрям | Що доступно |
| --- | --- |
| Головна | Збір 82 відібраних параметрів автомобіля, автоматичний запуск, поточний стан та інформація про базу |
| Всі дані | Дослідницький збір із каталогу 23 096 сигнатур читання, сирі значення та запис змін |
...
- Збирає 82 відібрані параметри автомобіля лише для читання.
...
Деякі поля мають відомі обмеження: сирий `CHARGING_STATE` недостовірний і не використовується для визначення заряджання в експорті чи Telegram. Заряджання визначається за підключеним зарядним пістолетом і додатною потужністю заряджання. Для `max_discharge_power_allow_raw` одиниця виміру та масштаб у кВт не підтверджені, а коди положення люка/шторки не можна трактувати просто як «відкрито/закрито».
...
- Читає повний каталог із 23 096 сигнатур лише для читання.
...
- Поточна поїздка містить відстань, тривалість, енергію, середню витрату й початковий/кінцевий SOC цього переїзду. Загальні значення охоплюють поточну сесію від увімкнення авто; SOC блоку «Загалом» — від першого переїзду до останнього показника, включно зі споживанням на стоянках.
...
Автоматична перевірка оновлень починається після короткої стартової паузи та виконується лише коли застосунок видимий. Закриття пропозиції оновлення призупиняє автоматичні перевірки на годину або до нової сесії застосунку. Ручна перевірка залишається доступною й завжди перевіряє останній опублікований реліз.
