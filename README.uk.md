...
Буфер callback-подій обмежений: кожен потік має спільний ліміт 4 MiB для даних у черзі та під час передачі, а файли опитування й callback-подій ділять його наявний ліміт резервного spool 128 MiB. Після переривання застосунок імпортує збережені пакети без дублів. Втрати через ліміт або помилки запису явно фіксуються; для даних, що ще в пам'яті, збереження після kernel reboot не гарантується.
