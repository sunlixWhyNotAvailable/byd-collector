...
| Діагностика | Ручний повносистемний logcat у `Опції -> Підтримка роботи`, локальна історія станів/помилок і поширення вибраних архівів |
...
- запуск або зупинка єдиного повносистемного запису logcat унизу `Підтримка роботи`;
...
Окремої вкладки `Логи` та дубльованого режиму Journal немає. Для відтворюваної проблеми запускайте й зупиняйте єдиний запис logcat унизу `Опції -> Підтримка роботи`. Запуск чекає завершеної перевірки авторизації ADB, а старт/стоп recorder разом зі snapshot/ZIP виконується поза UI-потоком. Використовується точний повносистемний виклик `logcat -b all -v threadtime`; тихого fallback до PID-фільтрованого app-only файла немає.
