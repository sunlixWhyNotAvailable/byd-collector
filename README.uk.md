...
| Діагностика | Ручний повносистемний logcat у `Опції -> Підтримка роботи`, локальна історія станів/помилок, системне поширення ZIP і безпечне очищення завершених логів |
...
- запуск або зупинка єдиного повносистемного запису logcat, поширення його діагностичного ZIP або очищення завершених логів унизу `Підтримка роботи`;
...
Окремої вкладки `Логи` та дубльованого режиму Journal немає. Для відтворюваної проблеми запускайте й зупиняйте єдиний запис logcat унизу `Опції -> Підтримка роботи`. `Поділитись логами` оновлює поточний діагностичний ZIP і відкриває системне Android-вікно поширення з незмінною cache-копією. `Очистити логи` видаляє завершені записи та сформовані пакети, не перериваючи активний запис, але захищає свіжу копію для поширення протягом 10 хвилин, щоб застосунок-одержувач устиг її прочитати. Прострочені копії прибираються наступною дією Share/Clear, а без неї можуть залишатися, доки Android не очистить cache застосунку. Старт/стоп recorder, snapshot/ZIP, підготовка поширення й очищення виконуються поза UI-потоком. Запуск чекає завершеної перевірки авторизації ADB. Використовується точний повносистемний виклик `logcat -b all -v threadtime`; тихого fallback до PID-фільтрованого app-only файла немає.
