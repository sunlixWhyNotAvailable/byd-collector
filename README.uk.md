...
Персональна тестова збірка v2.8.3 використовує гібридний callback-збір. Після отримання придатних callback-подій підтримувані поля переходять на оновлення від автомобіля з контрольним читанням кожні п'ять секунд; непідтверджені поля залишаються на опитуванні. Power, передача та SOC зберігають звичайне швидке читання. Кожна отримана callback-подія, навіть із незмінним значенням, окремо зберігається в наявних базах SQLite. Перед ширшим релізом ще потрібна перевірка на авто.
