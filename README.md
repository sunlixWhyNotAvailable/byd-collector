...
Charging notifications use a connected charge gun plus positive charging power as the primary signal, with the vehicle charging-state field used only when that primary data is incomplete. After process recovery, a restored charging session requires a new continuous 60-second low-power interval before it can be declared stopped. Charging-progress messages report energy and SOC added both since the preceding configured progress step and across the whole charging session.
Trip summaries default to 10 seconds after parking and can be configured from 5 to 300 seconds. The parked snapshot is persisted and finalized at that deadline even when no later telemetry poll succeeds. After process recovery, an overdue snapshot waits for fresh confirmed gear; a resumed drive suppresses the stale summary, while a 30-second fallback is used only if telemetry never arrives. A summary is queued whenever SOC changes by at least 1%; otherwise it requires more than 0.1 km of distance or more than 0.1 kWh of trip energy. The message separates the current P-to-P trip from totals observed during the current vehicle boot.

The durable Telegram outbox preserves every event in strict FIFO order. Delivery attempts process only the oldest row, retain retry/backoff and permanent-block behavior at the head of the queue, and pace successful backlog messages by at least 5 seconds. A later trip or charging session does not discard earlier queued messages.
